...
def get_batch(batch, word_vec, sememe, emb_dim=300):
    ...
    sememe_data = np.zeros((max_len, len(batch), sememe.size()), dtype = np.float)
    #sememe_data = torch.zeros([max_len, len(batch), sememe.size()], dtype = torch.uint8).cuda()

    for i in range(len(batch)):
        for j in range(len(batch[i])):
            embed[j, i, :] = word_vec[batch[i][j]]
            for k in sememe.read_word_sememe(batch[i][j]):
                sememe_data[j, i, k] = 1.0/len(batch[i][j])
