...
class LSTM_sememe(nn.Module):
    def __init__(self, config):
        super(LSTM_sememe, self).__init__()
    ...
    def forward(self, inputs, length, emb_s):
        # hx: (child_c, child_h)
...
class NLINet(nn.Module):
    def __init__(self, config):
        ...
        self.sememesumlstm = SememeSumLstm(self.enc_lstm_dim, self.enc_lstm_dim)
        ...
        self.inputdim = self.inputdim/2 if self.encoder_type == "LSTMEncoder" or "LSTM_sememe"\
                                        else self.inputdim
    ...
    def forward(self, s1, s2):
        # s1 : (s1, s1_len)
        emb_s_1 = self.sememe_sum(s1[2])
        emb_s_2 = self.sememe_sum(s2[2])
        u = self.encoder(s1[0], s1[1], emb_s_1)
        v = self.encoder(s2[0], s2[1], emb_s_2)
