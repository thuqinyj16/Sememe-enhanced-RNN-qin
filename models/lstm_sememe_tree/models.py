...
class LSTM_sememe(nn.Module):
    ...
    def forward(self, inputs, length, sememe_c, sememe_h):
        # hx: (child_c, child_h)
...
class NLINet(nn.Module):
    ...
    def forward(self, s1, s2):
        # s1 : (s1, s1_len)
        s1_s_c, s1_s_h = self.sememe_sum(s1[2])
        s2_s_c, s2_s_h = self.sememe_sum(s2[2])
        u = self.encoder(s1[0], s1[1], s1_s_c, s1_s_h)
        v = self.encoder(s2[0], s2[1], s2_s_c, s2_s_h)
